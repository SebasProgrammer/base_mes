...
def get_json_file_of_variables(quarterly_variables:list) -> dict:
    
    file_obj = SharePoint().download_file(FILE_NAME, FOLDER_NAME)
    sheet = 'BASE_TRIM'
    base_trimestral_dataframe = pd.read_excel(
        BytesIO(file_obj), 
        sheet_name = sheet, 
        header = 0, 
        usecols = lambda variable : variable in quarterly_variables)
    ...
    with engine.connect() as connection:
        for column in base_trimestral_dataframe.columns:
            ...
            logger.info(f'Finding if {column} : {column_sql} is in any table')
            ...
            try:
                ...
                logger.info(f'\tResult of query is {table_names}')
                if table_names == []:
                    logger.warning(f'\t{column} with {column_sql} is not in any table')
                    continue

                for table in table_names:
                    if re.search(r'tri\b', table[0]):
                        table_name = table[0]
                        logger.info(f'\t{column_sql} is in {table_name}')
                        break
                    else:
                        logger.warning(f'There is no tri table, then table_name is assigned None')
                        table_name = None
            except:
                logger.warning(f'We are in the except now, table_name is assigned None')
                table_name = None
            logger.info(f'\tThe code found the table_name to be {table_name}')
            if table_name:
...
if __name__ == '__main__':
    ...
    quarterly_variables = get_quarterly_variables()

    with open('src/columns.json', 'w', encoding='utf-8') as file: 
        json.dump(quarterly_variables, file, ensure_ascii=False, indent=2)

    variables_dictionary_json = get_json_file_of_variables(quarterly_variables)
